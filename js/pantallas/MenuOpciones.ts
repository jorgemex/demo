...
export namespace MenuOpciones {
    ...
    export function fn_mostrar_ocultar_opciones(parent: d3.Selection<any, any, any, any>) {
        if (!isMnuOptnsShown) {
            if (mnu_opts == null) {
                mnu_opts = parent.append("g")
                    .style("pointer-events", "all");

                mnu_opts.append("rect")
                    .attr("x", "0")
                    .attr("y", "0")
                ...
                //Menu List
                let list_in = mnu_opts.append("g")
                    .style("fill", D3Utils.prop_color_acent)
                    .attr("transform", "translate(" + (MainPage._template.fun_get_width() / 2 - 11) + ", " + (D3Utils.prop_height_header + 3) + ")");
                ...
                fn_crear_tabla_lista(list_in2, _menu, MainPage._template.fun_get_width() / 2, _height_item_menu * _menu.length, D3Utils.prop_color_acent, fn_item_selected_options);
            } else {
    ...
    function fn_item_selected_options(datum?: Entidades.IMenu, index?: number, outerIndex?: number) {
    ...
    function fn_crear_tabla_lista(contenedor: d3.Selection<any, any, any, any>, data: Array<Entidades.IMenu>, wth_menu: number, hgt_menu: number, background_color: string, callback: VoidFunction) {
        prop_listaMenuD3 = Lista.fun_main({
            ...
            prop_config_fila: {
                ...
                fun_on_dibuja_fila: (g_fila: SVGGElement, datum: Entidades.IMenu, cx: number, cy: number) => {
                    fn_crear_fila_menu(d3.select(g_fila), datum);
                },
                fun_on_actualiza_fila: (g_fila: SVGGElement, datum: Entidades.IMenu, cx: number, cy: number) => {
                }
            }
        });
    ...
    function fn_crear_fila_menu(fila: d3.Selection<any, any, any, any>, datum: Entidades.IMenu): void {
