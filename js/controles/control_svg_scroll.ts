...
export namespace Scroll {
    ...
    enum tipo_scroll { x = 1, y = 2, contenido = 0 }
     ...
     * @param config
     */
    export function fun_main(svg_g_element: SVGGElement, config: iConfig): iObjetoControl {
        var propiedades: iProperties = {};
        propiedades.prop_es_dispositivo_movil = Utils.fn_EsMovil();
        propiedades.prop_config = config;
        propiedades.el_contenido_ = d3.select(svg_g_element);
        propiedades.el_contenedor_scroll = d3.select(document.createElementNS("http://www.w3.org/2000/svg", "g"))
            .classed("control-scroll", true)
            .property("propiedades", propiedades)
            .on("wheel", function () { fun_on_wheel(propiedades); })
        ...
        var obj_control: iObjetoControl = {
            ...
            , fun_set_scrollTop: (posicion: number) => { fun_set_scroll_top(propiedades, posicion); }
            , fun_get_scrollTop: () => fun_get_scroll_top(propiedades)
            , fun_set_scrollLeft: (posicion: number) => { fun_set_scroll_left(propiedades, posicion); }
    ...
    function fun_init_properties(propiedades: iProperties): void {
        ...
        if (propiedades.prop_config.prop_width === undefined) propiedades.prop_config.prop_width = 0;
        if (propiedades.prop_config.prop_height === undefined) propiedades.prop_config.prop_height = 0;
        if (propiedades.prop_config.prop_height_total === undefined) propiedades.prop_config.prop_height_total = 0;
        ...
        if (propiedades.prop_config.mostrar_scroll_x) {
            if (propiedades.prop_config.prop_width_total) width_total = propiedades.prop_config.prop_width_total;
        ...
        //
        propiedades.prop_aux_scroll_x = (propiedades.prop_config.mostrar_scroll_x && propiedades.prop_width < width_total) ? true : false;
        propiedades.prop_aux_scroll_y = (propiedades.prop_config.mostrar_scroll_y && propiedades.prop_height < propiedades.prop_config.prop_height_total) ? true : false;
        // 
        propiedades.prop_height_total = propiedades.prop_aux_scroll_y ? propiedades.prop_config.prop_height_total : propiedades.prop_height;
        propiedades.prop_width_total = propiedades.prop_aux_scroll_x ? width_total : propiedades.prop_width;

        if (propiedades.prop_config.prop_stroke_width === undefined) propiedades.prop_config.prop_stroke_width = 5;
        if (propiedades.prop_config.prop_stroke === undefined) propiedades.prop_config.prop_stroke = "#000000";
        if (!propiedades.prop_config.event_scroll) propiedades.prop_config.event_scroll = {};

        // Inicializa valores para el scroll X.
        if (propiedades.prop_config.mostrar_scroll_x) {
            if (!propiedades.prop_scroll_x) propiedades.prop_scroll_x = {};
            if (!propiedades.prop_scroll_x.prop_scale_linear) propiedades.prop_scroll_x.prop_scale_linear = d3.scaleLinear();
            if (!propiedades.prop_scroll_x.prop_posicion) propiedades.prop_scroll_x.prop_posicion = { x: 0, y: 0, index: tipo_scroll.x };

            propiedades.prop_scroll_x.prop_scale_linear.range([0, propiedades.prop_width]).domain([0, propiedades.prop_width_total]);
        }
        ...
        if (propiedades.prop_config.mostrar_scroll_y) {
            ...
            if (!propiedades.prop_scroll_y.prop_scale_linear) propiedades.prop_scroll_y.prop_scale_linear = d3.scaleLinear();
            if (!propiedades.prop_scroll_y.prop_posicion) propiedades.prop_scroll_y.prop_posicion = { x: 0, y: 0, index: tipo_scroll.y };

            propiedades.prop_scroll_y.prop_scale_linear.range([0, propiedades.prop_height]).domain([0, propiedades.prop_height_total]);
        }

        if (!propiedades.prop_drag) {
            propiedades.prop_drag = d3.drag()
                .on("start", function (d: any) { fun_on_drag_start(propiedades, this); })
                .on("drag", function (d: any) { fun_on_drag(propiedades, this); })
                .on("end", function (d: any) { fun_on_drag_end(propiedades, this); });
        }

        // Inicializa el indice del control. Para distinguir clipath usado.
        if (propiedades.prop_index_control === undefined) propiedades.prop_index_control = prop_index_control_scroll += 1;
    }
    ...
    function fun_clipPath(propiedades: iProperties): void {
        ...
        if (el_clip_scroll_rect.size() === 0) {
            el_clip_scroll_rect = propiedades.el_contenedor_scroll.append("defs").append("clipPath")
                .attr("id", "clip_scroll_" + propiedades.prop_index_control)
                .append("rect").attr("class", "clip-scroll-rect");
        }

        el_clip_scroll_rect.attr("width", propiedades.prop_width)
    ...
    function fun_dibuja_scroll(propiedades: iProperties): void {
        ...
        g_scroll.style("opacity", .6).attr("pointer-events", "all");
        ...
        if (propiedades.prop_config.mostrar_scroll_x) {
            ...
            propiedades.prop_scroll_x.prop_posicion.y = propiedades.prop_height - propiedades.prop_config.prop_stroke_width;
            // Barra del scroll.
            propiedades.prop_scroll_x.el_barra = <any>g_scroll.select(".scroll-x")
        ...
        if (propiedades.prop_config.mostrar_scroll_y) {
            ...
            propiedades.prop_scroll_y.prop_posicion.x = propiedades.prop_width - propiedades.prop_config.prop_stroke_width;
            // Barra del scroll.
            propiedades.prop_scroll_y.el_barra = <any>g_scroll.select(".scroll-y")
    ...
    function fun_agregar_contenido(propiedades: iProperties): void {
        ...
        if (el_g_contenido_clip.size() === 0) {
            ...
            propiedades.el_contenedor_scroll.append("rect")
            ...
            el_g_contenido_clip = propiedades.el_contenedor_scroll.append("g")
            ...
            el_g_contenido_clip.append("rect")
                .classed("contenido-background", true)
                .attr("fill", propiedades.prop_config.prop_background_color || "#ffffff");

            // Se agrega el elemento SVGGElement al grupo que tiene el clipPath.
            el_g_contenido_clip.datum(null).append(() => propiedades.el_contenido_.node()).attr("transform", "translate(0, 0)");
        }

        propiedades.el_contenedor_scroll.select(".scroll-background")
            .attr("width", propiedades.prop_config.prop_width)
            .attr("height", propiedades.prop_config.prop_height);

        el_g_contenido_clip.select(".contenido-background")
     ...
     * @param transicion
     */
    function fun_asignar_top_scroll(propiedades: iProperties, _tipo_scroll: tipo_scroll, dx: number, dy: number, invert: boolean, transicion: boolean = false): void {
    ...
    function fun_escalar_posicion_scroll(propiedades: iProperties): void {
        if (!propiedades.prop_height || !propiedades.prop_width || !propiedades.prop_config.prop_height || !propiedades.prop_config.prop_width) return;
        ...
        if (propiedades.prop_scroll_y) {
            ...
            propiedades.prop_scroll_y.prop_posicion.y = (posicion_contenido / (propiedades.prop_config.prop_height_total || 1)) * propiedades.prop_config.prop_height;
        }
        // Escalar la posicion que tiene el scroll horizontal con las nuevas medidas.
        if (propiedades.prop_scroll_x) {
            propiedades.prop_scroll_x.prop_posicion.x = (propiedades.prop_scroll_x.prop_posicion.x / propiedades.prop_width) * propiedades.prop_config.prop_width;
        }
    }

    function fun_translate_scroll_x(propiedades: iProperties, pos_x: number, tipo_evento: number, invert: boolean, transition: boolean = false): void {
        // Si esta activado el scroll se calcula la posicion X y traslada el contenido.
        if (propiedades.prop_config.mostrar_scroll_x && (tipo_evento === tipo_scroll.contenido || tipo_evento === tipo_scroll.x)) {
            pos_x *= !invert ? -1 : 1;
            ...
            if (!transition) propiedades.prop_scroll_x.el_barra.attr("x", propiedades.prop_scroll_x.prop_posicion.x);
            else propiedades.prop_scroll_x.el_barra./* transition().duration(200).ease(d3.easeLinear). */attr("x", propiedades.prop_scroll_x.prop_posicion.x);
        }
    }

    function fun_translate_scroll_y(propiedades: iProperties, pos_y: number, tipo_evento: number, invert: boolean, transition: boolean = false): void {
        // Si esta activado el scroll se calcula la posicion Y y traslada el contenido.
        if (propiedades.prop_config.mostrar_scroll_y && (tipo_evento === 0 || tipo_evento === tipo_scroll.y)) {
            pos_y *= !invert ? -1 : 1;
            ...
            if (!transition) propiedades.prop_scroll_y.el_barra.attr("y", propiedades.prop_scroll_y.prop_posicion.y);
            else propiedades.prop_scroll_y.el_barra./* transition().duration(200).ease(d3.easeLinear). */attr("y", propiedades.prop_scroll_y.prop_posicion.y);
        }
    }

    function fun_translate_contenido(propiedades: iProperties, t_x: number, t_y: number, transition: boolean = false): void {
        fun_mostrar_scroll(propiedades, true);
        fun_timer_ocultar_scroll(propiedades);
        var posicion_contendido: iCoordenada = { x: 0, y: 0, index: tipo_scroll.contenido };
        ...
        if (!transition) propiedades.el_contenido_.attr("transform", "translate(" + posicion_contendido.x + ", " + posicion_contendido.y + ")");
        else propiedades.el_contenido_./* transition().duration(200).ease(d3.easeLinear). */attr("transform", "translate(" + posicion_contendido.x + ", " + posicion_contendido.y + ")");

        // Ejecutar callBack del evento scroll.
        if (propiedades.prop_config.event_scroll && propiedades.prop_config.event_scroll.scroll) propiedades.prop_config.event_scroll.scroll();
    }

    function fun_timer_ocultar_scroll(propiedades: iProperties): void {
        if (!propiedades.prop_evento_mouse_enter_activo) {
            if (propiedades.prop_timer_ocultar_scroll != undefined) clearTimeout(propiedades.prop_timer_ocultar_scroll);
    ...
    function fun_mostrar_scroll(propiedades: iProperties, toggle: boolean): void {
        if (propiedades.prop_scroll_esta_visible === toggle
            || propiedades.prop_config.prop_no_ocultar) return;

        propiedades.prop_scroll_esta_visible = toggle;
        propiedades.el_contenedor_scroll.select(".g-scroll-contenido")
            .attr("pointer-events", toggle ? "all" : "none").interrupt()
            .transition().duration(100).style("opacity", toggle ? .6 : 0)
    ...
    function fun_actualizar(): void {
        ...
        var propiedades = <iProperties>d3.select(obj_control.element).property("propiedades");
    ...
    function fun_set_scroll_top(propiedades: iProperties, posicion: number): void {
        var posicion_contenido = propiedades.prop_scroll_y.prop_scale_linear.invert(propiedades.prop_scroll_y.prop_posicion.y);
    ...
    function fun_set_scroll_left(propiedades: iProperties, posicion: number): void {
    ...
    // FUNCIONES - EVENTOS
    // -----------------------------------------------------------------------------------
    function fun_on_drag_start(propiedades: iProperties, elemento: Element): void {
    ...
    function fun_on_wheel(propiedades: iProperties): void {
        ...
        var pos_y = propiedades.prop_scroll_y.prop_scale_linear((propiedades.prop_height / 4)) * (value > 0 ? -1 : 1);
