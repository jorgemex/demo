...
const d3 = require("d3");
const Utils_1 = require("../comun/Utils");
var Scroll;
(function (Scroll) {
    ...
    function fun_main(svg_g_element, config) {
        var propiedades = {};
        propiedades.prop_es_dispositivo_movil = Utils_1.Utils.fn_EsMovil();
        ...
        var obj_control = {
            ...
            fun_set_scrollTop: (posicion) => { fun_set_scroll_top(propiedades, posicion); },
            fun_get_scrollTop: () => fun_get_scroll_top(propiedades),
            fun_set_scrollLeft: (posicion) => { fun_set_scroll_left(propiedades, posicion); },
    ...
    function fun_init_properties(propiedades) {
        ...
        if (propiedades.prop_config.mostrar_scroll_x) {
            ...
            if (!propiedades.prop_scroll_x.prop_posicion)
                propiedades.prop_scroll_x.prop_posicion = { x: 0, y: 0, index: tipo_scroll.x };
        ...
        if (propiedades.prop_config.mostrar_scroll_y) {
            ...
            if (!propiedades.prop_scroll_y.prop_posicion)
                propiedades.prop_scroll_y.prop_posicion = { x: 0, y: 0, index: tipo_scroll.y };
        ...
        if (!propiedades.prop_drag) {
            propiedades.prop_drag = d3.drag()
                .on("start", function (d) { fun_on_drag_start(propiedades, this); })
                .on("drag", function (d) { fun_on_drag(propiedades, this); })
                .on("end", function (d) { fun_on_drag_end(propiedades, this); });
        }
    ...
    function fun_dibuja_scroll(propiedades) {
        ...
        if (propiedades.prop_config.mostrar_scroll_x) {
            ...
            propiedades.prop_scroll_x.el_barra = g_scroll.select(".scroll-x")
        ...
        if (propiedades.prop_config.mostrar_scroll_y) {
            ...
            propiedades.prop_scroll_y.el_barra = g_scroll.select(".scroll-y")
    ...
    function fun_escalar_posicion_scroll(propiedades) {
        if (!propiedades.prop_height || !propiedades.prop_width || !propiedades.prop_config.prop_height || !propiedades.prop_config.prop_width)
            return;
        if (propiedades.prop_scroll_y) {
    ...
    function fun_translate_contenido(propiedades, t_x, t_y, transition = false) {
        ...
        var posicion_contendido = { x: 0, y: 0, index: tipo_scroll.contenido };
        ...
        if (!transition)
            propiedades.el_contenido_.attr("transform", "translate(" + posicion_contendido.x + ", " + posicion_contendido.y + ")");
        else
            propiedades.el_contenido_.attr("transform", "translate(" + posicion_contendido.x + ", " + posicion_contendido.y + ")");
        if (propiedades.prop_config.event_scroll && propiedades.prop_config.event_scroll.scroll)
