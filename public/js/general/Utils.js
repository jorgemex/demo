...
(function (D3Utils) {
    ...
    function fn_crear_encabezado_atras(callBack) {
        let g_header = d3.select(document.createElementNS("http://www.w3.org/2000/svg", "g"));
        g_header.attr("transform", "translate(0,0)");
        g_header
            .append("rect")
            .classed("head_rect", true)
            .attr("width", "100%")
            .attr("height", D3Utils.prop_height_header)
        ...
        g_header_back
            ...
            .attr("fill", D3Utils.prop_color_text_header)
        ...
        g_header_content
            ...
            .attr("x", "50")
            .attr("y", "30")
            .style("fill", D3Utils.prop_color_text_header)
            .style("font-size", "18");
        return g_header;
    }
    D3Utils.fn_crear_encabezado_atras = fn_crear_encabezado_atras;
    function fn_crear_encabezadoV1() {
        let g_header = d3.select(document.createElementNS("http://www.w3.org/2000/svg", "g"));
        g_header.attr("transform", "translate(0,0)");
        g_header
            .append("rect")
        ...
        g_header_back
            .append("path")
            .attr("d", "M 3 5 A 1.0001 1.0001 0 1 0 3 7 L 21 7 A 1.0001 1.0001 0 1 0 21 5 L 3 5 z M 3 11 A 1.0001 1.0001 0 1 0 3 13 L 21 13 A 1.0001 1.0001 0 1 0 21 11 L 3 11 z M 3 17 A 1.0001 1.0001 0 1 0 3 19 L 21 19 A 1.0001 1.0001 0 1 0 21 17 L 3 17 z")
            .attr("fill", D3Utils.prop_color_text_header)
    ...
    D3Utils.fn_crear_encabezadoV1 = fn_crear_encabezadoV1;
    function fn_crear_encabezado() {
        let g_header = d3.select(document.createElementNS("http://www.w3.org/2000/svg", "g"));
        g_header.attr("transform", "translate(0,0)");
        g_header
            .append("rect")
        ...
        let g_header_opts = g_header_content
            .append("g")
            .attr("transform", "translate(" + (MainPage_1.MainPage._template.fun_get_width() - 50) + ",0)")
        ...
        g_header_opts
            .append("path")
            .attr("d", "M24 16c2.21 0 4-1.79 4-4s-1.79-4-4-4-4 1.79-4 4 1.79 4 4 4zm0 4c-2.21 0-4 1.79-4 4s1.79 4 4 4 4-1.79 4-4-1.79-4-4-4zm0 12c-2.21 0-4 1.79-4 4s1.79 4 4 4 4-1.79 4-4-1.79-4-4-4z")
            .attr("fill", D3Utils.prop_color_text_header)
            .attr("transform", "translate(5,9) scale(0.7)");
        return g_header;
    }
    D3Utils.fn_crear_encabezado = fn_crear_encabezado;
    function fn_crear_piedepagina(callBack) {
        D3Utils._width_footer_container = MainPage_1.MainPage._template.fun_get_width() - 50;
        let g_footer = d3.select(document.createElementNS("http://www.w3.org/2000/svg", "g"));
        g_footer.attr("transform", "translate(0,0)");
        g_footer
            .append("rect")
            .classed("footer_rect", true)
            .attr("width", "100%")
            .attr("height", D3Utils.prop_height_footer)
            .attr("fill", "#FFFFFF");
        ...
        g_footer_content
            ...
            .attr("width", D3Utils._width_footer_container)
            .attr("height", D3Utils.prop_height_footer)
            .attr("fill", "#FFFFFF");
        let g_footer_back = g_footer
            .append("g")
            .attr("transform", "translate(" + D3Utils._width_footer_container + ",0)")
    ...
    D3Utils.fn_crear_piedepagina = fn_crear_piedepagina;
    function fn_actualizar_pie_pagina(g_footer, to_show) {
        g_footer.select(".footer_rect").attr("height", D3Utils.prop_height_footer);
        g_footer.select("fotter_back_rect").attr("height", D3Utils.prop_height_footer);
    ...
    D3Utils.fn_actualizar_pie_pagina = fn_actualizar_pie_pagina;
    function fn_crear_footer_button(text) {
        D3Utils._width_footer_button = D3Utils._width_footer_container / 2 - 10;
        D3Utils._height_footer_button = 34;
    ...
    D3Utils.fn_crear_footer_button = fn_crear_footer_button;
    function fn_crear_footer_more_button(contenedor) {
        let g_content_btn = contenedor.append("g");
        var width_button = D3Utils._width_footer_container / 2 - 10;
    ...
    D3Utils.fn_crearBtn = fn_crearBtn;
    function met_toMSJSON(dt1) {
        var date = "/Date(" + dt1.getTime() + ")/";
        return date;
    }
    D3Utils.met_toMSJSON = met_toMSJSON;
    function fun_es_dispositivo_movil() {
    ...
    D3Utils.fun_es_dispositivo_movil = fun_es_dispositivo_movil;
    function wrap(el_texto, width, font_size, char, num_filas, sangria, wrap_word = false) {
        let v_dy = font_size / 3 + font_size;
        if (font_size)
        ...
        el_texto.each(function (d, i) {
            let text = d3.select(this), words = text.text().split(/\s+/), word, line = [], lineNumber = 0, lineHeight = 1.1, x = text.attr("x") || 0, y = text.attr("y"), dy = parseFloat(text.attr("dy")), cont_filas = 0, tspan = text
                .text(null)
    ...
    D3Utils.wrap = wrap;
    function fun_limitar_texto(el_texto, width, padding_, char) {
        el_texto.each(function () {
            var self = d3.select(this), textLength = self.node().getComputedTextLength(), text = self.text();
    ...
    D3Utils.fun_limitar_texto = fun_limitar_texto;
    function fun_wrap_word(el_texto, width) {
    ...
    D3Utils.fun_wrap_word = fun_wrap_word;
    let D3DateFormat;
    (function (D3DateFormat) {
        D3DateFormat.prop_formato_fecha_hora = "%d/%m/%Y %I:%M %p";
        D3DateFormat.prop_formato_fecha = "%d/%m/%Y";
    })(D3DateFormat = D3Utils.D3DateFormat || (D3Utils.D3DateFormat = {}));
    let D3Font;
    (function (D3Font) {
        D3Font.prop_font = "Arial, Verdana, Tahoma";
    })(D3Font = D3Utils.D3Font || (D3Utils.D3Font = {}));
    let eventos;
    (function (eventos) {
        function fun_agregar_click(seleccion, callBack, nombre) {
            if (fun_es_dispositivo_movil()) {
                if (!callBack) {
                    ...
                    seleccion
                        .on("touchstart." + (nombre || ""), function () {
                        fun_on_click_down.bind(this)();
                    }, false)
                        .on("touchend." + (nombre || ""), function (datum, index, allElement) {
                ...
                if (!callBack)
                    seleccion.on("click." + (nombre || ""), null);
                else
                    seleccion.on("click." + (nombre || ""), function (datum, index, allElement) {
                        fun_on_click_up.bind(this)(callBack, datum, index, allElement);
                    }, false);
            }
        }
        eventos.fun_agregar_click = fun_agregar_click;
        function fun_on_click_down() {
            d3.select(this).property("init_evento_pos", d3.mouse(document.body));
            d3.select(this).property("init_evento_time", new Date().getTime());
        }
        function fun_on_click_up(fnc_callback_evento, datum, index, allElement) {
            ...
            let posicion_elemento = this.getBoundingClientRect();
            if ((!init_evento_time && !init_evento_pos) ||
        ...
        function fun_ejecutar_click(elemento) {
            if (fun_es_dispositivo_movil())
                elemento.dispatch("touchend");
            else
                elemento.dispatch("click");
        }
        eventos.fun_ejecutar_click = fun_ejecutar_click;
    })(eventos = D3Utils.eventos || (D3Utils.eventos = {}));
})(D3Utils = exports.D3Utils || (exports.D3Utils = {}));
